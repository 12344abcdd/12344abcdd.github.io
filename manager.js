...
async function fetchTokenSuffix() {
    ...
    if (!res.ok) { throw new Error("æ æ³è·å token ååé¨å"); }
...
async function initApp() {
    try {
        ...
        if (userRes.ok) {
            ...
            document.getElementById("ghRepoListTitle").textContent = `${owner} çä»åºåè¡¨`;
        }
    } catch (e) {
        console.error("è·åç¨æ·ä¿¡æ¯å¤±è´¥:", e);
    }
    updateHeaderInfo();
    loadFiles();
}
...
function showPath() {
    document.getElementById("ghPath").innerHTML =
        '<span style="color:#d97706"><b>å½åè·¯å¾ï¼</b></span>' +
        (curPath ? curPath : '/');
}
function showActions() {
    document.getElementById("ghTopActions").innerHTML = `
        ...
        <button onclick="showUploadFile()" class="save-btn">ä¸ä¼ æä»¶</button>
        <button onclick="showUploadFolder()" class="save-btn">ä¸ä¼ æä»¶å¤¹</button>
    `;
}
...
async function loadFiles(path="") {
    ...
    if (!res.ok) {
        document.getElementById("ghFiles").innerHTML = `<tr><td colspan="3">æ æ³è¯»åæä»¶åè¡¨</td></tr>`;
        showActions();
        return;
    }
    ...
    if (path) {
        ...
        html += `<tr>
            <td>â©ï¸</td>
            <td class="file-name"><a href="javascript:void(0)" onclick="goDir('${upPath}')">è¿åä¸çº§</a></td>
            <td></td>
        </tr>`;
    }
    for (const f of files) {
        ...
        html += `<tr>
            ...
            ${f.type === "file" ? `
                ...
                <button onclick="delFile('${f.path}','${f.sha}')" class="del-btn">å é¤</button>
            ...
            ` : `
                ...
                <button onclick="delDir('${f.path}')" class="del-btn">å é¤</button>
...
// åç¼©åä¸ªæä»¶å¹¶ä¸ä¼  zip å°ä»åº
window.compressFile = async function(path) {
    showStatus("æ­£å¨åç¼©æä»¶...", "#0969da");
    ...
    const uploadRes = await fetch(uploadUrl, {
        ...
        body: JSON.stringify({
            message: `åç¼© ${path} å° ${zipPath} via manager.html`,
            content: zipBase64
        })
    });
    if (uploadRes.ok) {
        showStatus("åç¼©å¹¶ä¸ä¼ æåï¼");
        await loadFiles(curPath);
    } else {
        showStatus("åç¼©ä¸ä¼ å¤±è´¥","#cf222e");
    }
};

// åç¼©æ´ä¸ªç®å½å¹¶ä¸ä¼  zip å°ä»åº
window.compressFolder = async function(path) {
    showStatus("æ­£å¨æåç®å½...", "#0969da");
    ...
    const uploadRes = await fetch(uploadUrl, {
        ...
        body: JSON.stringify({
            message: `åç¼©ç®å½ ${path} å° ${zipPath} via manager.html`,
            content: zipBase64
        })
    });
    if (uploadRes.ok) {
        showStatus("ç®å½åç¼©å¹¶ä¸ä¼ æåï¼");
        await loadFiles(curPath);
    } else {
        showStatus("ç®å½åç¼©ä¸ä¼ å¤±è´¥","#cf222e");
    }
};

// è§£å zip æä»¶å¹¶ä¸ä¼ å°ä»åº
let unzipProgress = {
    total: 0,
    current: 0,
    isRunning: false,
...
window.decompressFile = async function(path) {
    ...
    showStatus("æ­£å¨è§£åå¹¶ä¸ä¼ ...", "#0969da");
    const url = `https://api.github.com/repos/${owner}/${repo}/contents/${path}`;
    const res = await fetch(url, {
        headers: {
            "Authorization": token ? "token " + token : undefined,
    ...
    document.getElementById("ghUnzipStatus").textContent = `æ­£å¨è§£åä¸ä¼ æä»¶ï¼å±${unzipProgress.total}ä¸ªï¼...`;
    ...
    for (let i = 0; i < relPaths.length; i++) {
        ...
        const uploadRes = await fetch(uploadUrl, {
            ...
            body: JSON.stringify({
                message: `è§£å ${path} èªå¨ä¸ä¼  ${relPath} via manager.html`,
                content: fileContent
            })
        });
        ...
        document.getElementById("ghUnzipStatus").textContent = `æ­£å¨è§£åä¸ä¼ æä»¶ï¼${unzipProgress.current} / ${unzipProgress.total}`;
    }

    unzipProgress.isRunning = false;
    document.getElementById("ghUnzipStatus").textContent = `è§£åå®æ: ${uploadCount} æä»¶, å¤±è´¥: ${failCount}`;
    decompressBtns.forEach(btn => {
        btn.textContent = "è§£å";
        btn.disabled = false;
        btn.classList.remove("del-btn");
...
// å¶å® window.xxx æ¹æ³è¯·ç¨ä½ çåå§ manager.js åå®¹ä¿æå®æ´ã

// åªè´´æ¿å¯¹è±¡ï¼æ¯ææä»¶/ç®å½è·¨ä»åºå¤å¶ç²è´´ï¼
let clipboard = { type: "", path: "", repo: "", owner: "" };

// å¤å¶æé®äºä»¶ï¼æä»¶æç®å½ï¼
window.copyItem = function(path, type) {
    clipboard.type = type;
    clipboard.path = path;
    clipboard.repo = repo;
    clipboard.owner = owner;
    showStatus(`å·²å¤å¶${type === "dir" ? "ç®å½" : "æä»¶"}: ${path}ï¼${repo}ï¼`);
};

// ç²è´´æé®äºä»¶ï¼ç®æ ä»åºãç®æ ç¨æ·ãç®æ ç®å½ï¼
window.pasteItem = async function(targetRepo, targetOwner, targetDir) {
    if (!clipboard.path) return showStatus("è¯·åå¤å¶æä»¶æç®å½", "#cf222e");
    if (clipboard.type === "file") {
        ...
        if (!res.ok) return showStatus("è¯»åæºæä»¶å¤±è´¥", "#cf222e");
        const data = await res.json();
        ...
        const uploadRes = await fetch(uploadUrl, {
            ...
            body: JSON.stringify({
                message: `å¤å¶æä»¶å° ${targetRepo}:${destPath} via manager.html`,
                content: base64Content
            })
        });
        if (uploadRes.ok) showStatus("ç²è´´æåï¼");
        else showStatus("ç²è´´å¤±è´¥","#cf222e");
    } else if (clipboard.type === "dir") {
        showStatus("æ­£å¨å¤å¶ç®å½ï¼è¯·ç­å¾ï¼...", "#0969da");
        // éå½å¤å¶ç®å½
            ...
            for (const item of items) {
                if (item.type === "dir") {
                    ...
                    await fetch(uploadUrl, {
                        ...
                        body: JSON.stringify({
                            message: `å¤å¶ç®å½å° ${dstRepo}:${destPath} via manager.html`,
                            content: base64Content
                        })
                    });
                }
...
//
    ...
    window.showRepoList = async function() {
        ...
        document.getElementById("ghRepoList").innerHTML = "æ­£å¨å è½½...";
        token = localStorage.getItem("gh_token") || "";
        if (!token) {
            token = await getFullToken();
            localStorage.setItem("gh_token", token);
        ...
        if (!res.ok) {
           document.getElementById("ghRepoList").innerHTML = "æ æ³è·åä»åºåè¡¨";
            return;
        }
        const repos = await res.json();
        let html = `<ul>`;
        for (const repoItem of repos) {
            if (repoItem.owner.login !== owner) continue;  // åªæ¾ç¤ºèªå·±çä»åº
            html += `<li>
        ...
        html += `
            <div class="gh-repo-actions">
                ...
                <button onclick="showSettings()" class="save-btn" disabled>ä»åºè®¾ç½®</button>
            </div>
        `;
    ...
    window.switchRepo = function(newRepo) {
        ...
        updateHeaderInfo();
        loadFiles();
        showStatus("å·²åæ¢å°ä»åº: " + newRepo, "#0969da");
    };
    ...
    window.createRepo = async function() {
        ...
        showStatus("æ­£å¨åå»ºä»åº...", "#0969da");
        ...
        const data = await res.json();
        if (res.ok) {
            showStatus("ä»åºåå»ºæåï¼");
            document.getElementById("newRepoName").value = "";
            await showRepoList();
        } else {
            showStatus("åå»ºå¤±è´¥: " + (data.message || ""), "#cf222e");
        }
    };

    // ä»åºå¯¼å¥åè½
    window.showImportRepo = async function() {
        const vcs_url = prompt("è¯·è¾å¥è¦å¯¼å¥çä»åº URL:");
        if (!vcs_url) return;
        ...
        showStatus("æ­£å¨å¯¼å¥ä»åº...", "#0969da");
        ...
        if (res.ok) {
            showStatus("ä»åºå¯¼å¥å·²å¼å§ï¼è¯·ç¨åå·æ°æ¥ç");
        } else {
            showStatus("å¯¼å¥å¤±è´¥ï¼è¯·æ£æ¥ URL æ¯å¦æ­£ç¡®", "#cf222e");
        }
    };
    ...
    window.showSettings = async function() {
        ...
        document.getElementById("ghSettingsContent").innerHTML = "å è½½ä¸­...";
        ...
        if (res.ok) {
                ...
                <div>
                    <label>
                        <input type="checkbox" id="isPrivate" ${data.private ? 'checked' : ''} 
                            onclick="updateRepoSetting('private', this.checked)"> ç§æä»åº
                    </label>
                </div>
                <div>
                    <label>
            ...
            document.getElementById("ghSettingsContent").innerHTML = "å è½½è®¾ç½®å¤±è´¥";
        }
    };
    ...
    window.openFullScreen = async function(path, sha) {
        ...
        const ext = path.split('.').pop().toLowerCase();
        let topActions = "";
        if (["md","txt","json","js","ts","css","html","py","java","c","cpp","go","rs","php","yaml","gradle","yml"].includes(ext) || ext.length <= 5) {
            topActions = `<button onclick="saveFullScreenEdit()" class="save-btn">ä¿å­</button>
                <button onclick="closeFullScreen()" class="del-btn">åæ¶</button>`;
        }
        ...
        if (commitRes.ok) {
            ...
            if (commitData.length > 0) {
                ...
                commitInfoHtml = `<div style="color:#57606a;font-size:0.98em;margin-bottom:8px;">
                    æè¿æäº¤: ${commit.commit.message}<br/>
        ...
        if (["png","jpg","jpeg","gif","bmp","svg","webp"].includes(ext)) {
            ...
            contentHtml += `<a href="${rawUrl}" download="${getFileName(path)}" class="save-btn" target="_blank">ä¸è½½æå¨æ°çªå£æ¥çæä»¶</a>`;
        }
        document.getElementById("ghFullContent").innerHTML = commitInfoHtml + contentHtml;
        if (lastFullScreenType === "text") {
            document.getElementById("ghFullscreenEdit").value = "æ­£å¨å è½½åå®¹...";
    ...
    window.saveFullScreenEdit = async function() {
        ...
        if (res.ok) {
            ...
            if (data.message && data.message.includes("Bad credentials")) {
                ...
                token = "";
                showStatus("Token å¤±æææ æï¼è¯·æ£æ¥ token.txt æä»¶ã", "#cf222e");
            }
        }
    }
            // æä»¶éå½å
    ...
    window.doRenameFile = async function() {
        ...
        if (getFileName(renameOldPath) === newName) return showStatus("æä»¶åæªä¿®æ¹", "#cf222e");
        ...
        if (renameType === "dir") {
            ...
            if (ok) {
                showStatus("ç®å½éå½åæåï¼");
                await loadFiles(curPath);
                closeRenameModal();
            } else {
                showStatus("ç®å½éå½åå¤±è´¥", "#cf222e");
            }
        } else {
            ...
            if (!res.ok) {
                showStatus("è·ååæä»¶åå®¹å¤±è´¥", "#cf222e");
                return;
            }
            const data = await res.json();
            ...
            if (createRes.ok) {
                ...
                if (deleteRes.ok) {
                    showStatus("éå½åæåï¼");
                    await loadFiles(curPath);
                    closeRenameModal();
                } else {
                    showStatus("éå½ååå é¤åæä»¶å¤±è´¥: " + (deleteData.message || ""), "#cf222e");
                }
            } else {
                showStatus("éå½åå¤±è´¥: " + (createData.message || ""), "#cf222e");
            }
        }
    };
            function getFileName(path) {
    ...
    window.delFile = async function(path, sha) {
        if (!confirm(`ç¡®å®è¦å é¤æä»¶ï¼${path} åï¼`)) return;
        token = localStorage.getItem("gh_token") || "";
        if (!token) {
            token = await getFullToken();
            localStorage.setItem("gh_token", token);
        }
        showStatus("æ­£å¨å é¤...", "#cf222e");
        const url = `https://api.github.com/repos/${owner}/${repo}/contents/${path}`;
        const res = await fetch(url, {
            method: "DELETE",
            headers: {
        ...
        const data = await res.json();
        if (res.ok) {
            showStatus("å é¤æåï¼");
            await loadFiles(curPath);  // ç¡®ä¿ç®å½ç«å³å·æ°
        } else {
            showStatus("å é¤å¤±è´¥: " + (data.message || ""), "#cf222e");
            if (data.message && data.message.includes("Bad credentials")) {
                localStorage.removeItem("gh_token");
                token = "";
                showStatus("Token å¤±æææ æï¼è¯·æ£æ¥ token.txt æä»¶ã", "#cf222e");
    ...
    window.delDir = async function(path) {
        if (!confirm(`ç¡®å®è¦å é¤ç®å½åå¶ææåå®¹ï¼${path} åï¼`)) return;
        token = localStorage.getItem("gh_token") || "";
        if (!token) {
            token = await getFullToken();
            localStorage.setItem("gh_token", token);
        }
        showStatus("æ­£å¨å é¤ç®å½...", "#cf222e");
        ...
        if (failed === 0) {
            showStatus("ç®å½å é¤æåï¼");
            await loadFiles(curPath);  // ç¡®ä¿ç®å½ç«å³å·æ°
        } else {
            showStatus(`ç®å½å é¤å®æï¼ä½æ ${failed} ä¸ªæä»¶å é¤å¤±è´¥`, "#cf222e");
        }
    }
    ...
    document.getElementById('ghUploadInput').onchange = async function(evt) {
        ...
        reader.onload = async function(e) {
            ...
            const data = await res.json();
            if (res.ok) {
                showStatus("ä¸ä¼ æåï¼");
                await loadFiles(curPath);  // ç¡®ä¿ç®å½ç«å³å·æ°
            } else {
                showStatus("ä¸ä¼ å¤±è´¥: " + (data.message || ""), "#cf222e");
                if (data.message && data.message.includes("Bad credentials")) {
                    localStorage.removeItem("gh_token");
                    token = "";
                    showStatus("Token å¤±æææ æï¼è¯·æ£æ¥ token.txt æä»¶ã", "#cf222e");
    ...
    document.getElementById('ghUploadFolderInput').onchange = async function(evt) {
        ...
        showStatus("æ­£å¨ä¸ä¼ æä»¶å¤¹...", "#0969da");
        ...
        showStatus(`ä¸ä¼ å®æï¼${count} ä¸ªæä»¶ï¼å¤±è´¥ï¼${failed} ä¸ªã`);
        await loadFiles(curPath);  // ç¡®ä¿ç®å½ç«å³å·æ°
    };
    ...
    window.showNewFile = function() {
        document.getElementById("ghTopActions").innerHTML = `
            ...
            <button onclick="createFile()" class="save-btn">åå»º</button>
            <button onclick="cancelEdit()" class="del-btn">åæ¶</button>
        `;
    }
    ...
    window.showNewDir = function() {
        document.getElementById("ghTopActions").innerHTML = `
            ...
            <button onclick="createDir()" class="save-btn">åå»º</button>
            <button onclick="cancelEdit()" class="del-btn">åæ¶</button>
        `;
    }
    ...
    window.createDir = async function() {
        ...
        showStatus("æ­£å¨åå»ºç®å½...", "#0969da");
        const url = `https://api.github.com/repos/${owner}/${repo}/contents/${filePath}`;
        const res = await fetch(url, {
            method: "PUT",
            headers: {
